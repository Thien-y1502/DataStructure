...
struct Node {
    int id;
    vector<pair<Node*, double>> children; // List of child nodes with edge weight.
    Node* parent = nullptr;
...
private:
    Node* root;
    double B;  // Energy limit per route.
    ...
    void assignDistanceFromRoot(Node* node, double currentDistance, unordered_set<int>& visitedSet) {
        if (visitedSet.count(node->id))
            return;
        visitedSet.insert(node->id);
        distanceFromRoot[node->id] = currentDistance;
        for (auto& childPair : node->children) {
            Node* child = childPair.first;
            double w = childPair.second;
            assignDistanceFromRoot(child, currentDistance + w, visitedSet);
        }
    }
...
public:
    // Constructs the tree from an edge list. Each tuple represents (from, to, weight).
    Tree(const vector<tuple<int, int, double>>& edgeList, int n, double _B) : B(_B) {
        if (n == 0) throw invalid_argument("Empty edge list");
        root = getOrCreate(0);
    ...
    // Returns the path from a given node back to the root.
    ...
    // PDFS_offline implements a recursive DFS (piecemeal-DFS) with an energy limit.
    // When the energy of proceeding would exceed B, the current route is ended (with a return path to the root)
    // and a new route begins. In addition to printing the route information and cost, the process of DFS traversal
    // (including the full DFS order with backtracking) is printed.
    void PDFS_offline() {

        // Variables for tracking the DFS process and route segmentation.
        // 'path' stores the nodes from the root to the current node (with the weight of the edge from its parent).
        vector<pair<Node*, double>> path;
        // 'currentRoute' holds the list of node IDs visited in the current route.
        vector<int> currentRoute;
        // 'routes' holds all the routes recorded.
        vector<vector<int>> routes;
        // 'routeCosts' stores the total cost for each completed route.
        vector<double> routeCosts;

        // Variables for tracking energy and distance.
        double currentDistance = 0.0; // Distance from the root to the current node.
        double routeEnergy = 0.0;     // Energy used in the current route (cumulative sum of distances).
        double sumEnergy = 0.0;       // Total energy accumulated (per the DFS process).
        int routeCount = 1;           // Route numbering (starting from 1).

        // 'fullDFSOrder' records the DFS traversal events (including both visitation and backtracking).
        vector<int> fullDFSOrder;

        // Initialize with the root node.
        path.push_back({ root, 0.0 });
        currentRoute.push_back(root->id);
        fullDFSOrder.push_back(root->id);

        // Recursive DFS function using a lambda.
        function<void(Node*)> DFS = [&](Node* u) {
            ...
            for (auto& childPair : u->children) {
                ...
                double w = childPair.second;
                if (!v->visited) {
                    // Check if moving from u to v will exceed the energy limit.
                    if (routeEnergy + w + (currentDistance + w) > B) {
                        // End the current route at node u and return to root.
                        double distToRoot = currentDistance;  // Distance from root to u.
                        routeEnergy += distToRoot;
                        cout << "\nEnd route " << routeCount << ": back to node " << u->id
                            << ", length = " << distToRoot
                            << ", total energy S = " << sumEnergy << endl;
                            // Complete the current route using the return path stored in 'path'.
                        ...
                        for (int i = 1; i < (int)path.size(); i++) {
                            ...
                            routeEnergy += wt;
                            sumEnergy += newDist;
                            cout << "From node " << parentNode->id
                                << " to node " << curNode->id
                                << ", weight = " << wt
                                << ", distance from root = " << newDist
                                << ", sum of weight = " << newDist
                                << ", sum S = " << sumEnergy << endl;
                    ...
                    fullDFSOrder.push_back(v->id);
                    currentDistance += w;
                    routeEnergy += w;
                    sumEnergy += currentDistance;
                    cout << "From node " << u->id
                        << " to node " << v->id
                        << ", weight = " << w
                        << ", distance from root = " << currentDistance
                        << ", sum of weight = " << currentDistance
                        << ", sum S = " << sumEnergy << endl;
        ...
        if (currentRoute.size() > 1) {
            double distToRoot = currentDistance; // Normally 0 if we are at root.
            routeEnergy += distToRoot;
             cout << "End route " << routeCount
                 << ": return to root from node " << path.back().first->id
                 << ", weight = " << distToRoot
                 << ", sum S = " << sumEnergy << endl;
            for (int i = (int)path.size() - 2; i >= 0; i--) {
                currentRoute.push_back(path[i].first->id);
        ...
        // Print the full DFS traversal order (with backtracking events).
        cout << "\nTHE FULL DFS TRAVERSAL (with backtracking): ";
        for (int id : fullDFSOrder) {
            cout << id << " ";
        }
        cout << endl;

        //Print the final routes along with the cost for each route.
       cout << "\n=== Final Routes ===" << endl;
...
// Reads the edge list using an edge-list data structure.
// First, the number of nodes is read, then (n-1) edges in the format: from, to, weight.
...
int main() {
    try {
        ...
        // Build the tree from the edge list and run the PDFS_offline algorithm.
        Tree tree(edgesList, n, B);
        cout << "\n=== Starting PDFS Offline ===" << endl;
        tree.PDFS_offline();
    }
...
Route 2 (cost = 40): 0 1 2 19 20 21 22 21 20 19 2 1 0 23 25 26 27 28 27 26 29 26 25 23 24 30 31
32 33 34 35 36 35 34 33 32 31 30 24 23 0
