...
public:
	...
	void PDFS_offline() {
		...
		double currentLength = 0.0; // Distance from the root to the current node.
			...
			for (auto& childPair : u->children) {
				...
				if (!v->visited) {
					...
					if (routeEnergy + w + (currentLength + w) > B) {
						...
						double distToRoot = currentLength;  // Distance from root to u.
		...
		if (currentRoute.size() > 1) {
			double distToRoot = currentLength; // Normally 0 if we are at root.
			routeEnergy += distToRoot;
			for (int i = (int)path.size() - 2; i >= 0; i--) {
				currentRoute.push_back(path[i].first->id);
			}
